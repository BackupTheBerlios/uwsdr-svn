...
#define emacTX_BUF_USED				( ( u32 ) 0x80000000 )
...
#define emacRESET_KEY				( ( u32 ) 0xA5000000 )
#define emacRESET_LENGTH			( ( u32 ) ( 0x01 << 8 ) )
 ...
 */
static void EMAC_SetupInterrupt(void);

/*
 * Some initialisation functions taken from the Atmel EMAC sample code.
 */
static void ETH_ReadPHY( u8 ucPHYAddress, u8 ucAddress, u8 *pulValue );
#if USE_RMII_INTERFACE != 1
	static void vWritePHY( u8 ucPHYAddress, u8 ucAddress, u32 ulValue);
#endif
static char xGetLinkSpeed( void );
...
static volatile s8 pcRxBuffer[ NB_RX_BUFFERS * ETH_RX_BUFFER_SIZE ];

/* Buffer read by the EMAC DMA.  Must be aligned as described by he comment
above the emacADDRESS_MASK definition. */
#pragma data_alignment=8
static s8 pcTxBuffer[ NB_TX_BUFFERS * ETH_TX_BUFFER_SIZE ];
...
const s8 cMACAddress[ 6 ] = { _MAC_ADDR_0, _MAC_ADDR_1, _MAC_ADDR_2, _MAC_ADDR_3, _MAC_ADDR_4, _MAC_ADDR_5 };
const unsigned char ucIPAddress[ 4 ]  = { _IP_ADDR_0, _IP_ADDR_1, _IP_ADDR_2, _IP_ADDR_3 };
...
{
  static unsigned char uxTxBufferIndex = 0;
  char xWaitCycles = 0;
  s32 lReturn = pdPASS;
  s8 *pcBuffer;
  ...
  while(!( xTxDescriptors[ uxTxBufferIndex ].U_Status.status & AT91C_TRANSMIT_OK )) {
    ...
    if( xWaitCycles > emacMAX_WAIT_CYCLES )	{
      ...
      delay_us( emacBUFFER_WAIT_DELAY );
    }
  }
  ...
  if( lReturn == pdPASS ) {
    ...
    pcBuffer = ( s8 * ) xTxDescriptors[ uxTxBufferIndex ].addr;
    //memcpy( ( void * ) pcBuffer, ( void * ) m_pTxBuf, MAX(tx_len, _NET_MIN_PACKET_SIZE) );
    memcpy( ( void * ) pcBuffer, ( void * ) m_pTxBuf, tx_len );
    
    /* If there is room, also copy in the application data if any. */
//    if( ( tx_len > emacTOTAL_FRAME_HEADER_SIZE ) && ( uip_len <= ( ETH_TX_BUFFER_SIZE - emacTOTAL_FRAME_HEADER_SIZE ) ) )
//    {
//      memcpy( ( void * ) &( pcBuffer[ emacTOTAL_FRAME_HEADER_SIZE ] ), ( void * ) uip_appdata, ( tx_len - emacTOTAL_FRAME_HEADER_SIZE ) );
//    }
    ...
    //portENTER_CRITICAL();
    {
      if( uxTxBufferIndex >= ( NB_TX_BUFFERS - 1 ) )
      {
        ...
        xTxDescriptors[ uxTxBufferIndex ].U_Status.status = ( tx_len & ( u32 ) AT91C_LENGTH_FRAME )
          | AT91C_LAST_BUFFER
            | AT91C_TRANSMIT_WRAP;
        uxTxBufferIndex = 0;
      }
...
s16 EMAC_Poll( void )
{
  static unsigned char ulNextRxBuffer = 0;
  u32 ulSectionLength = 0, ulLengthSoFar = 0, ulEOF = pdFALSE;
  s8 *pcSource;
  s16 result = _NET_OK;
  
  _DBG_STATE_POS(_DBG_STATE_ETH);
  ...
  while( ( xRxDescriptors[ ulNextRxBuffer ].addr & AT91C_OWNERSHIP_BIT )
        && !ulSectionLength ) {
    pcSource = ( s8 * )( xRxDescriptors[ ulNextRxBuffer ].addr & emacADDRESS_MASK );
    ...
    if( ulSectionLength == 0 ) {
      ...
      if( ( ulLengthSoFar + ETH_RX_BUFFER_SIZE ) <= NET_BUFSIZE ) {
        memcpy((u8*)m_pRxBuf + ulLengthSoFar, pcSource, ETH_RX_BUFFER_SIZE );
      ...
      uIP. */
      if( ulSectionLength < NET_BUFSIZE ) {
        ...
        if( ulSectionLength > ulLengthSoFar ) {
          memcpy((u8*)m_pRxBuf + ulLengthSoFar, pcSource, ( ulSectionLength - ulLengthSoFar ) );
        }
      }			
      result = _NET_GOT_RX_DATA;
...
static void prvSetupDescriptors(void)
{
  ...
  for( xIndex = 0; xIndex < NB_RX_BUFFERS; ++xIndex ) {
    ...
    ulAddress = ( u32 )( pcRxBuffer + ( xIndex * ETH_RX_BUFFER_SIZE ) );
  ...
  for( xIndex = 0; xIndex < NB_TX_BUFFERS; ++xIndex ) {
    /* Calculate the address of the nth buffer within the array. */
    ulAddress = ( u32 )( pcTxBuffer + ( xIndex * ETH_TX_BUFFER_SIZE ) );
  ...
  AT91C_BASE_EMAC->EMAC_RBQP = ( u32 ) xRxDescriptors;
  AT91C_BASE_EMAC->EMAC_TBQP = ( u32 ) xTxDescriptors;
...
static void prvSetupMACAddress( void )
{
  ...
  AT91C_BASE_EMAC->EMAC_SA1L =	( ( u32 ) cMACAddress[ 3 ] << 24 ) |
    ( ( u32 ) cMACAddress[ 2 ] << 16 ) |
      ( ( u32 ) cMACAddress[ 1 ] << 8  ) |
        cMACAddress[ 0 ];
  
  AT91C_BASE_EMAC->EMAC_SA1H =	( ( u32 ) cMACAddress[ 5 ] << 8 ) |
    cMACAddress[ 4 ];
}
/*-----------------------------------------------------------*/
...
static void EMAC_SetupInterrupt(void)
{
  ...
  AT91F_AIC_ConfigureIt( AT91C_BASE_AIC, AT91C_ID_EMAC, emacINTERRUPT_LEVEL, AT91C_AIC_SRCTYPE_INT_HIGH_LEVEL, ( void (*)( void ) ) vEMACISR );
...
__arm void vEMACISR( void )
{
volatile u32 ulIntStatus, ulRxStatus;
char xSwitchRequired = pdFALSE;
...
static char prvProbePHY( void )
{
  ...
  /* Read the PHY ID. */
  EMAC_ReadPHY( AT91C_PHY_ADDR, MII_PHYSID1, &ulPHYId1 );
  EMAC_ReadPHY( AT91C_PHY_ADDR, MII_PHYSID2, &ulPHYId2 );
...
static char xGetLinkSpeed( void )
{
	u32 ulBMSR, ulBMCR, ulLPA, ulMACCfg, ulSpeed, ulDuplex;
	...
	{				
		...
		EMAC_ReadPHY(AT91C_PHY_ADDR, MII_LPA, &ulLPA);
...
//
//****************************************************************************
void EMAC_copyRxPayload(u8* pTX, u8* pRX, u16 offset, u16 length)
{
  //*** DEFINITON ***
  int i;
  u8  xIndex;
  ...
  i = offset/ETH_RX_BUFFER_SIZE;
  
  //search start descriptor of this frame
