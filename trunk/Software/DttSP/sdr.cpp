...
setup_all (REAL rate,
	   int buflen,
	   SDRMODE mode, int specsize, int numrecv, int cpdsize)
{
...
setup_tx (void)
{
  ...
  tx.leveler.gen = newDttSPAgc (agcLONG,	// mode kept around for control reasons
  ...
  tx.alc.gen = newDttSPAgc (agcLONG,	// mode kept around for control reasons alone
			    CXBbase (tx.buf.i),	// input buffer
			    CXBsize (tx.buf.i),	// output buffer
			    1.2f,	// Target output 
			    2,	// Attack time constant in ms
		 ...
		 int specsize, int numrecv, int cpdsize)
{
  ...
  setup_all (rate, buflen, mode, specsize, numrecv, cpdsize);
