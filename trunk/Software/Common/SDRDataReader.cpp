...
void CSDRDataReader::clock()
{
	...
	unsigned long availSamples = m_buffer->dataSpace();

	if(availSamples < m_blockSize)
		return;
...
bool CSDRDataReader::readSocket()
{
	...
	if (m_sequence != -1 && seqNo != m_sequence) {
		...
		if (seqNo < m_sequence) {
			::wxLogWarning(wxT("SDRDataReader: Packet dropped at sequence no: %d, expected: %d. Ssocket len: %d"), seqNo, m_sequence, len);
			return true;
		} else {
			::wxLogWarning(wxT("SDRDataReader: Packet missed at sequence no: %d, expected: %d. Socket len: %d"), seqNo, m_sequence, len);
		}
		// catch up
		m_sequence = seqNo;
	}

	// I removed the wrap around algo, a wrap around will happen every 2 seconds,
	// no packet will live that long in a local network.
	// the simple counter saves some processing overhead in the backend.
	...
	if(nSamples != 400) {
		::wxLogWarning(wxT("SDRDataReader: Malformed size: %d length %d"), seqNo, len);
	}

	// I used these platform indipendent data types in the backend.
	...
	for (u32 i = 0; i < nSamples && n < len; n += SAMPLE_SIZE, i++) {
		iData = ((u8)m_sockBuffer[n + 0] << 24);
		iData |= ((u8)m_sockBuffer[n + 1] << 16);
		iData |= (m_sockBuffer[n + 2] << 8);

		qData = ((u8)m_sockBuffer[n + 3] << 24);
		qData |= ((u8)m_sockBuffer[n + 4] << 16);
		qData |= ((u8)m_sockBuffer[n + 5] << 8);

		float buffer[2];
		//buffer[0] = float(iData) / 8388607.0F - 1.0F;
		//buffer[1] = float(qData) / 8388607.0F - 1.0F;
		buffer[0] = (s32)iData / (float)0x80000000;
		buffer[1] = (s32)qData / (float)0x80000000; 
