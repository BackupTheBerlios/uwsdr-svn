...
bool CHackRFController::open()
{
	...
	for (unsigned int index = 0U; ::SetupDiEnumDeviceInterfaces(devInfo, NULL, &guid, index, &devInfoData); index++) {
		...
		if (attributes.VendorID  == HACKRF_VID && attributes.ProductID == HACKRF_PID) {
			...
			ULONG sent = 0UL;
			ret1 = ::WinUsb_ControlTransfer(m_handle, packet, NULL, 0UL, &sent, NULL);
			if (!ret1) {
...
bool CHackRFController::setFrequency(const CFrequency& freq)
{
	...
	ULONG length = sizeof(setFreqParams);
	...
	ULONG sent = 0UL;
	BOOL result = ::WinUsb_ControlTransfer(m_handle, packet, (UCHAR *)&setFreqParams, length, &sent, NULL);
	if (!result) {
		::wxLogError(wxT("Error from WinUsb_ControlTransfer: err=%lu"), ::GetLastError());
		return false;
	}
...
bool CHackRFController::setTransmit(bool tx)
{
	...
	packet.RequestType = (BMREQUEST_HOST_TO_DEVICE << 7) | (BMREQUEST_VENDOR << 5) | BMREQUEST_TO_DEVICE;
	packet.Request = REQUEST_SET_TRANSMIT;
	packet.Value = tx ? 2U : 1U;
	packet.Index = 0U;
	packet.Length = 0U;

	ULONG sent = 0UL;
	BOOL result = ::WinUsb_ControlTransfer(m_handle, packet, NULL, 0UL, &sent, NULL);
	if (!result) {
		::wxLogError(wxT("Error from WinUsb_ControlTransfer: err=%lu"), ::GetLastError());
		return false;
	}
...
void CHackRFController::close()
{
	...
	ULONG sent = 0UL;
	BOOL ret = ::WinUsb_ControlTransfer(m_handle, packet, NULL, 0UL, &sent, NULL);
	if (!ret)
		::wxLogError(wxT("Error from WinUsb_ControlTransfer: err=%lu"), ::GetLastError());
