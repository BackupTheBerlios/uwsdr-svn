...
bool CUWSDRController::open()
{
	...
	wxString hostAddrStr;

	m_peerAddress.Hostname(m_address);
	m_peerAddress.Service(m_port);

	sockAddress.AnyAddress();
	sockAddress.Service(m_port);

	//m_socket = new wxSocketClient(wxSOCKET_NOWAIT);
	m_socket = new wxDatagramSocket(sockAddress, wxSOCKET_NOWAIT);
    ...
    // really ugly, but I nneded it fast !!
	sendCommand("SD192.168.1.4;"); // The Computer's ethernet IP
...
{
	m_socket->SendTo(m_peerAddress, command, ::strlen(command));
}
